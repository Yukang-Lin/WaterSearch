...
from transformers import GenerationConfig, LogitsProcessor, DynamicCache
from sentence_transformers import SentenceTransformer, util


class TextMetrics:
    def __init__(self, tokenizer, device, embedder_model_path):
        self.tokenizer = tokenizer
        self.device = device
        self.semsim_model = SentenceTransformer(embedder_model_path, device=device)
        
    def calculate_semsim(self, reference: str, candidate: str) -> float:
        embeddings = self.semsim_model.encode([reference, candidate], convert_to_tensor=True)
...
class ChunkSearchGenerator():
    def __init__(self, args, tokenizer, model, dataset_name) -> None:
        ...
        embedder_model_path = args.embedder_model_path
        self.metrics = TextMetrics(tokenizer, self.device, embedder_model_path)
            ...
            while total_generated < max_new_tokens:
                ...
                for i, watermark_outputs in enumerate(watermark_outputs_list):
                    if watermark_outputs is None or len(watermark_outputs['tokens']) == 0:
                        continue
                    rouge_l = self.metrics.calculate_semsim(
